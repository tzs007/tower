<?php include("includes/head.php") ?>

	<div id="tower-homepage" class="hidden-ie">

		<?php include("includes/header.php"); ?>

			<main class="hero">
				<div class="owl-carousel">
					<div class="owl-carousel-item" style="background-image:url(resources/images/hero1.jpg)">
						<div class="container">
							<div class="row">
								<div class="col-sm-9 col-sm-offset-3 col-lg-8 col-lg-offset-4">
									<hgroup>
										<h1 class="hero-title">Profit from your Budapest property</h1>
										<h2 class="hero-subtitle">
											Budapest's <span>leading</span> property services group<br>
											<span>will make sure</span> your Hungarian property is<br>
											well maintained and protected.
										</h2>
									</hgroup>
									<ul>
										<li>Over 700 properties managed</li>
										<li>98% occupancy on managed stock</li>
										<li>10 years experience Range of services - from purchase to re-sale</li>
										<li>Over 90% foreign owned properties</li>
										<li>Experienced, multi langual staff</li>
									</ul>
									<p>
										<a href="#contact" class="btn btn-lg btn-cta text-uppercase roller">Contact us</a>
										<span class="or-separator">or</span><a href="landlord.php" class="learn-more">learn more</a>
									</p>
								</div>
							</div>
						</div>
					</div>
					<div class="owl-carousel-item" style="background-image:url(resources/images/hero2.jpg);padding:0">
						<a href="#" class="hotspot" style="width:100%,height:inherit;display:block;background-color:rgba(123,234,213,.5)"></a>
					</div>
				</div>
			</main>

			<section class="landlord-services block">
				<div class="container">
					<div class="row">
						<div class="col-sm-12">
							<h1 class="text-center landlord-services-title">
								<span class="linea left"></span>
								<span class="linea right"></span>
								Landlord Services
							</h1>
							<div class="row">
								<div class="col-sm-4 text-center landlord-service">
									<div class="icon-container icon-container-push-down"><i class="icon icon-presentation"></i></div>
									<h2 class="landlord-services-subtitle kill-border">Wealth Protection</h2>
									<p>Insurance, tenant screening, regular inspections, eviction of the non-paying tenant. Your property is <span class="important">maintained and protected.</span></p>
								</div>
								<div class="col-sm-4 text-center landlord-service">
									<div class="icon-container icon-container-push-down"><i class="icon icon-wallet" style="line-height:85px"></i></div>
									<h2 class="landlord-services-subtitle kill-border">Yield</h2>
									<p>Rental department, occupancy, bank transfers, cash collection, marketing. Let us make your investments <span class="important">work for you.</span></p>
								</div>
								<div class="col-sm-4 text-center landlord-service">
									<div class="icon-container icon-container-push-down"><i class="icon icon-chat" style="line-height:85px"></i></div>
									<h2 class="landlord-services-subtitle kill-border">Communication</h2>
									<p>Professional customer care, account management, Tower365 administration page for <span class="important">24/7 access</span> to your Budapest property.</p>
								</div>
								<div class="col-sm-12 text-center block-cta">
									<p><a href="landlord.php" class="btn btn-info btn-lg">Learn more</a></p>
									<p class="or-separator">or</p>
								</div>

								<div class="col-sm-10 col-sm-offset-1 get-in-touch" id="contact">

									<hgroup class="text-center">
										<h2 class="kill-margin get-in-touch-title">Get in touch now</h2>
										<h3 class="kill-margin get-in-touch-subtitle">and we will contact you within 24 hours</h3>
									</hgroup>

									<form action="" class="form-horizontal">
										<div class="form-group">
											<label for="name" class="col-sm-4 control-label input-lg">Your name: <span class="required">*</span></label>
											<div class="col-sm-8">
												<input type="text" class="form-control input-lg" id="name" placeholder="e.g: John Smith jr." data-validation="required">
											</div>
										</div>
										<div class="form-group">
											<label for="mobile" class="col-sm-4 control-label input-lg">Your mobile number: <span class="required">*</span></label>
											<div class="col-sm-8">
												<input type="password" class="form-control input-lg" id="mobile" placeholder="e.g: +36 20 123 4567" data-validation="required">
											</div>
										</div>
										<div class="form-group">
											<label for="email" class="col-sm-4 control-label input-lg">Your E-mail address: <span class="required">*</span></label>
											<div class="col-sm-8">
												<input type="email" class="form-control input-lg" id="email" placeholder="e.g: john.smith.jr@gmail.com" data-validation="email required">
											</div>
										</div>
										<div class="form-group">
											<label for="message" class="col-sm-4 control-label input-lg">Your message:</label>
											<div class="col-sm-8">
												<textarea name="message" id="message" cols="30" rows="3" class="form-control" placeholder=""></textarea>
											</div>
										</div>
										<div class="form-group">
											<div class="col-sm-offset-4 col-sm-8">
												<button type="submit" class="btn btn-success btn-lg">Send enquiry</button>
												<p class="form-control-static input-lg or-separator">or&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="about.php">Contact us</a> on <strong>+36 1 354 0980</strong></p>
											</div>
										</div>
									</form>
								</div>
							</div>
						</div>
					</div>
				</div>
			</section>

			<section class="infopanels block">
				<div class="container">
					<div class="row">
						<div class="col-sm-6 text-center infopanel left">
							<hgroup>
								<h2 class="infopanel-title">Budapest Rentals</h2>
								<h5 class="infopanel-subtitle">We have tenants waiting for your property</h5>
								<h6 class="infopanel-undertitle">Working investments</h6>
							</hgroup>
							<p class="infopanel-text">
								Through its' professional marketing channels and<br>
								team of experienced letting agents Tower rents out<br>
								dozen of properties each month, ensuring the highest<br>
								occupancy ratio for property investors.
							</p>
							<a href="rentals.php" class="btn btn-info btn-lg">Let your property</a>
						</div>
						<div class="col-sm-6 text-center infopanel right">
							<hgroup>
								<h2 class="infopanel-title">Property sales</h2>
								<h5 class="infopanel-subtitle">Selling at the right time. Selling at the right price.</h5>
								<h6 class="infopanel-undertitle">Selling with Tower</h6>
							</hgroup>
							<p class="infopanel-text">
								Free valuation &bull; Over 1000 properties sold &bull; 10 year's experience<br>
								Multi-langual staff &bull; No fee, all free advertisements!<br>
								Extensive marketing exposure<br>
								Independent lawyers, specialized in property law
							</p>
							<a href="sales.php" class="btn btn-info btn-lg">Sell your property</a>
						</div>
					</div>
				</div>
			</section>

			<?php include('includes/client-testimonial.php'); ?>

			<section class="tower365 block">
				<div class="left-image hidden-xs"></div>
				<div class="right-image hidden-xs"></div>
				<div class="container">
					<div class="row">
						<div class="col-sm-6 col-sm-offset-3 text-center">
							<hgroup>
								<h2 class="tower365-title light kill-border kill-margin">Tower 365</h2>
								<h3 class="tower365-subtitle kill-border">24/7 access to your Budapest property</h3>
							</hgroup>
							<p class="tower365-text">Through Tower's unique login system you will get 24/7 access to your
Budapest property. See financial transactions, up-to-date activity notes,
your tax returns, your property details
and more...</p>
							<a href="//demo.tower365.hu" class="btn btn-demo btn-lg">Demo</a>
						</div>
					</div>
				</div>
			</section>

			<section class="live-map block">
				<div class="container">
					<div class="row">
						<div class="col-sm-12 text-center">
							<div class="icon-container icon-container-push-down"><i class="icon icon-map" style="line-height:85px"></i></div>
							<hgroup>
								<h1 class="live-map-title kill-margin kill-border">
									<span class="linea left"></span>
									<span class="linea right"></span>
									Live map about our listed properties
								</h1>
								<h5 class="live-map-subtitle kill-margin">That map shows you properties for sale and for rent. Click on them for details.</h5>
							</hgroup>
							<div class="row pin-guide clearfix">
								<div class="col-sm-4 col-sm-offset-4">
									<div class="rent">
										<img src="resources/images/pin-guide-rent.png" alt="" class="pin">
										<strong>For Rent</strong>
									</div>
									<div class="sale">
										<img src="resources/images/pin-guide-sale.png" alt="" class="pin">
										<strong>For Sale</strong>
									</div>
								</div>
							</div>
						</div>
					</div>
				</div>
				<div id="map-canvas" class="map map-lg">map</div>
			</section>

			<footer class="bottom">
				<?php include("includes/footer-extended.php"); ?>
				<?php include("includes/footer-navigation.php"); ?>
			</footer>

		</div>

		<!-- call script -->

		<script>
			App.owl();
			App.formLabelColorizer();
			$.validate();
		</script>
	</body>
</html>
